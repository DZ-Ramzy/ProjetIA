class Puissance4:
    ...
    def jouer(self, col):
        """Joue un pion avec gestion de la gravité"""
    ...
    def victoire(self):
        """verifie si le joueur a gagné ou non"""
    ...
    def horiz(self, lig, col):
        """verifie si on a 4 elements du meme joueur alignes horizontalement"""
        return col <= self.m - 4 and all(self.grille[lig][col + i] == self.joueur for i in range(4))

    def vert(self, lig, col):
        """verifie si on a 4 elements du meme joueur alignes verticalement"""
        return lig <= self.n - 4 and all(self.grille[lig + i][col] == self.joueur for i in range(4))

    def diag_haut(self, lig, col):
        """verifie si on a 4 elements du meme joueur forme une diagonale vers le haut de la grille"""
    ...
    def diag_bas(self, lig, col):
        """verifie si on a 4 elements du meme joueur forme une diagonale vers le bas de la grille"""
        return lig >= 3 and col <= self.m - 4 and all(self.grille[lig - i][col + i] == self.joueur for i in range(4))

    def match_nul(self):
        """si toute les cases de la grille sont remplis et que personne n'a gagné"""
        return all(self.grille[-1][col] != 0 for col in range(self.m))

    def alterner_joueur(self):
        """switch entre le joueur 1 et le joueur 2 (sert quand on joue contre un pote en local)"""
